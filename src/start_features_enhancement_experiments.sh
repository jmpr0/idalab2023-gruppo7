...
function HELP {
  ...
  echo "${REV}-C${NORM}  --Sets the value for option ${BOLD}base_cpu${NORM}. Default is ${BOLD}1${NORM}."
...
while getopts "e:c:C:b:i:s:t:k:h" opt; do
  case $opt in
    e)
      exp_name=$OPTARG
      ;;
...
nomemapprs=("scratch")
memapprs=()
# hdrnets=("Lopez17CNN" "Lopez17CNNRNN")
#hdrnets=("Lopez17CNN")
loadnets=()
mmnets=()
...
for k in ${seeds[@]}; do
	...
	for net in ${hdrnets[@]} ${loadnets[@]}; do
		...
		for appr in ${memapprs[@]} ${nomemapprs[@]}; do
			...
			ofs_cmd="--out-features-size $ofs"
			t=$(( cnt % (n_cpu * 30)  ))
			echo "sleep $t && ${script_cmd} ${expname_cmd} ${respath_cmd} ${dataset_cmd} ${input_cmd} ${nc_base_cmd} ${nc_incr_cmd} ${stop_task_cmd} ${num_tasks_cmd} ${config_cmd} ${ofs_cmd} ${network_cmd} ${approach_cmd} ${memory_cmd} ${momlr_cmd} ${seed_cmd}" >> "${command_file}"
			cnt=$(( cnt + 30 ))
...
if [[ $(basename "$n_cpu") == processors* ]]; then
  ...
  # TODO: change the "processors_${campaign_ts}" filename to "processors_${n_cpu}" when no experiment is running
	...
	if [ ! -f "$processors" ]; then
		for cpu in $(seq ${base_cpu} $(( base_cpu + n_cpu - 1 ))); do
