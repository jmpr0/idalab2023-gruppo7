...
class ExemplarsDataset(MemoryDataset):
    ...
    def __init__(self, transform, class_indices, num_exemplars=0, num_exemplars_per_class=0,
                 exemplar_selection='random', is_networking=False, base_class_index=0):
        super().__init__({'x': [], 'y': []}, transform, class_indices=class_indices, is_networking=is_networking)
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--exemplar-selection', default='random', type=str,
                            choices=['herding', 'random', 'entropy', 'distance', 'ensemble'],
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform, t=None, from_inputs=False):
        if self._is_active():
            self.images, self.labels = self.exemplars_selector(model, trn_loader, selection_transform, t, from_inputs)
