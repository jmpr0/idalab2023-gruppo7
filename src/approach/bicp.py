...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=250, lr=0.1, lr_min=1e-5, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 num_bias_epochs=200, T=2, lamb=-1, base_lamb=5., knowledge_distillation=None, **kwargs):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--base-lamb', default=5., type=float, required=False,
                            help='Base lambda for the forgetting-intransigence trade-off (default=%(default)s)')
    ...
    def criterion(self, t, outputs, targets, targets_old):
        ...
        # trade-off - the lambda from the paper of LUCIR if lamb=-1
        if self.lamb == -1:
            lamb = self.base_lamb * math.sqrt(sum([h.out_features for h in self.model.heads[:-1]])
                                               / self.model.heads[-1].out_features)
            return torch.nn.functional.cross_entropy(torch.cat(outputs, dim=1), targets) + lamb * loss_dist
        else:
