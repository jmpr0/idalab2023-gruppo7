...
class Appr(Inc_Learning_Appr):
    ...
    def compute_fisher_matrix_diag(self, trn_loader):
        ...
        # print(self.num_samples, trn_loader.batch_size, len(trn_loader.dataset))
        # print(n_samples_batches)
        ...
        for k, (images, targets) in enumerate(itertools.islice(trn_loader, n_samples_batches)):
            # print(k, images.shape)
            # print(targets)
            # print([b for b in trn_loader][k][1])
            # print([b for b in self.exemplars_dataset][k][1])
            # print('NEXT')
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        # print('train_loop', len(trn_loader.dataset))
        ...
        if len(self.exemplars_dataset) > 0 and t > 0:
            ...
            # print(type(trn_loader.dataset + self.exemplars_dataset))
            # print(type(self.exemplars_dataset))
            # print('exemplars_dataset', len(trn_loader.dataset))
    ...
    def post_train_process(self, t, trn_loader, val_loader):
        ...
        # print('post_train_process', len(trn_loader.dataset))
        # EXEMPLAR MANAGEMENT -- select training subset
        self.exemplars_dataset.collect_exemplars(self.model, trn_loader, val_loader.dataset.transform)

        # PATCH: because the original version takes the trn_loader w/o exemplars to compute the fisher info, we pick
        # the sole original trn_loader from the extended trn_loader (i.e. trn_loader.dataset.datasets[0]).
        if t > 0:
            trn_loader = torch.utils.data.DataLoader(trn_loader.dataset.datasets[0],
                                                     batch_size=trn_loader.batch_size,
                                                     shuffle=True,
                                                     num_workers=trn_loader.num_workers,
                                                     pin_memory=trn_loader.pin_memory)
