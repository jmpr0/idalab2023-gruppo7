...
class Appr(Inc_Learning_Appr):
    """Class implementing the PodNET (Pooled Outputs Distillation) approach
    described in https://arxiv.org/pdf/2004.13513.pdf
    ...
    def pre_train_process(self, t, trn_loader):
        ...
        # Replace each task head with an LSCLinear layer
        # In the code from SS-IL, when another task is added (increment), the oldest are grouped into one, resulting in
        # two LSCLinear, the aggregated for old classes, and the last for new classes.
        # We implement the same logic, by storing support variables
        if t in [0, 1]:
            in_features = self.model.heads[t].in_features
            out_features = self.model.heads[t].out_features

            self.model.heads[t] = LSCLinear(in_features, out_features)
        if t > 1:
            in_features = self.model.heads[0].in_features
            out_features = self.model.heads[0].out_features + self.model.heads[1].out_features
            new_classes = self.model.heads[2].out_features
            K = self.model.heads[0].fc.K

            new_fc = SplitLSCLinear(in_features, out_features1+out_features2, self.args.step_size)
    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                if not self.exemplar_means:
                    ...
                    hits_taw, hits_tag, outputs = self.classify(t, feats, targets)
        ...
        return total_loss / total_num, total_acc_taw / total_num, total_acc_tag / total_num, outputs_tot, targets_tot
