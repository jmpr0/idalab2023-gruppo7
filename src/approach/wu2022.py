...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=250, lr=0.1, lr_min=1e-5, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 num_bias_epochs=200, T=2, lamb=-1, knowledge_distillation=None, **kwargs):
    ...
    def criterion(self, t, outputs, targets, targets_old):
        ...
        if self.lamb == -1:
            ...
            return (1.0 - lamb) * torch.nn.functional.cross_entropy(
                torch.cat(outputs, dim=1), targets, label_smoothing=.1) + lamb * loss_dist
        else:
            return torch.nn.functional.cross_entropy(
                torch.cat(outputs, dim=1), targets, label_smoothing=.1) + self.lamb * loss_dist
