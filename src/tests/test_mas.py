...
def test_mas_exemplars_lopezcnn_2feats17():
    args_line = FAST_LOCAL_TEST_ARGS + LOPEZ_CNN2_CONFIG
    args_line += " --num-exemplars 200"
    run_main_and_assert(args_line)


def test_mas_exemplars_lopez17rnn():
    args_line = FAST_LOCAL_TEST_ARGS + LOPEZ_RNN_CONFIG
    args_line += " --num-exemplars 200"
    run_main_and_assert(args_line)


def test_mas_exemplars_lopez17cnn():
    args_line = FAST_LOCAL_TEST_ARGS + LOPEZ_CNN2_CONFIG
    args_line += " --num-exemplars 200"
    run_main_and_assert(args_line)


def test_mas_exemplars_lopez17cnnrnn():
    args_line = FAST_LOCAL_TEST_ARGS + LOPEZ_CNN2RNN_CONFIG
    args_line += " --num-exemplars 200"
    run_main_and_assert(args_line)


def test_mas_exemplars_aceto19mimetic():
