...
class ResNet(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.layer1 = self._make_layer(block, math.ceil(scaling_factor*64), layers[0])
        self.layer2 = self._make_layer(block, math.ceil(scaling_factor*128), layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, math.ceil(scaling_factor*256), layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, math.ceil(scaling_factor*512), layers[3], stride=2, dilate=replace_stride_with_dilation[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(math.ceil(scaling_factor*512) * block.expansion, num_classes)
        self.head_var = 'fc'
